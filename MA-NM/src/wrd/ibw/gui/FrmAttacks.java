...
public class FrmAttacks extends JFrame {
	...
	public FrmAttacks(DBConnection pDBConnection) {
		this.dbConnection = pDBConnection;
		try {
		//setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
		this.setComponentOrientation(ComponentOrientation.LEFT_TO_RIGHT);
		this.setTitle("[Sardroudi & Ibrahim, 2010] Attack Simulator...");
		...
		btnStart.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					int markedTupl = dbConnection.getNoRows(cbTable.getSelectedItem().toString(), tfPrivateKey.getText(), Integer.valueOf(tfFractTupl.getText()));
					...
					CallableStatement gen_inf = dbConnection.getConnection().prepareCall ("{ ? = call GET_GENERAL_INFO (?,?,?,?,?)}");
					...
					gen_inf.setInt (5, 79);  // Generalizar esto
					gen_inf.setInt (6, 81);  // Generalizar esto
					gen_inf.execute ();
				    ...
				    int lsb_value = 0;				String binary_decimal = "";		String hav_value;
				    								int temp_decimal = 0;
				    
				    
		        	int msb_pos = 0;				boolean signed = false;			int real_value = 0;
		        	int msb_value = 0;				int absolute_value = 0;			int value_to_insert = 0;
		        	int cant_attack = 0; 			String prevID = "";
		        	int prop_attack = 0;
		        	int current_attack = 0;
		        	
		        	cant_attack = markedTupl * Integer.valueOf(txPercAttack.getText())/100;
		        	prop_attack = markedTupl/cant_attack;
				        	...
				        	for (int i = 0; i < attrToMark.size(); i++) {
					        	...
					        	if(new Integer(tfLSB.getText())*2 < binary_main.length()){
					        		if(rbOrigMeth.isSelected()){
					        			...
					        			attr_pos_cs.setInt (4, 79); //gneralizar esto
					        			attr_pos_cs.setInt (5, 81); //gneralizar esto
					        			attr_pos_cs.execute ();
					        		...
					        		if((msb_pos - 1) < (binary_main.length()-lsb_pos)){ //creo que ya lo salva la condicion de qeu LSB + MSB <= Binary.length
					        			...
					        			if((82 * (height_pos) + width_pos) > 0  ){
					        				
					        				current_attack++;
					        				if((current_attack > prop_attack)&&((rset_info.getString ("ID")) != prevID)){
					        					current_attack = 0;
			...
			cbMDist.setEnabled(false);
			cbMDist.setBounds(36, 113, 119, 23);
			pnAlgorithms.add(cbMDist);
			
			bgExtOpt.add(cbZDist);
			bgExtOpt.add(cbMDist);
			
			JLabel label_11 = new JLabel("Bits:");
			label_11.setHorizontalAlignment(SwingConstants.RIGHT);
			label_11.setBounds(30, 139, 52, 14);
			pnAlgorithms.add(label_11);
			
			txMinLSB = new JTextField();
			txMinLSB.setText("0");
			txMinLSB.setEditable(false);
			txMinLSB.setColumns(10);
			txMinLSB.setBounds(86, 136, 26, 20);
			pnAlgorithms.add(txMinLSB);
			
			cbLsbM = new JCheckBox("LSB");
			cbLsbM.setEnabled(false);
			cbLsbM.setBounds(114, 135, 52, 23);
